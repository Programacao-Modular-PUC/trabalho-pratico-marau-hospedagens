...
import com.marau.hospedagens.util.BercoRules;
import com.marau.hospedagens.util.CapacidadeHospedesRules;
...
public class QuartoService {
        ...
        public List<QuartoResponse> listar(String residencia, String tipo) {
                ...
                boolean temResidencia = residencia != null &&
                                !residencia.isBlank() &&
                                !residencia.equalsIgnoreCase("Todas");
                ...
                if (temResidencia && temTipo) {
                        quartos = quartoRepository.findByResidenciaNomeAndTipo(
                                        residencia,
                                        TipoQuarto.fromLabel(tipo));

                } else if (temResidencia) {
                        quartos = quartoRepository.findByResidenciaNome(residencia);

                } else if (temTipo) {
                        quartos = quartoRepository.findByTipo(
                                        TipoQuarto.fromLabel(tipo));
        ...
        public QuartoResponse criar(QuartoRequest req) {
                ...
                Residencia residencia = residenciaService.buscarEntidade(
                                req.residenciaId());
                ...
                Quarto q = Quarto.builder()
                                ...
                                .comodidades(
                                                resolverComodidades(req.comodidadeIds()))
                                .numCamas(
                                                req.numCamas() != null
                                                                ? req.numCamas()
                                                                : 1)
                                ...
                                .adicionalCamaExtra(
                                                req.adicionalCamaExtra() != null
                                                                ? req.adicionalCamaExtra()
                                                                : BigDecimal.ZERO)
                                .percentualHospede(
                                                req.percentualHospede() != null
                                                                ? req.percentualHospede()
                                                                : BigDecimal.ZERO)
                                .numeroHospedes(
                                                req.numeroHospedes() != null
                                                                ? req.numeroHospedes()
                                                                : 1)
                                .build();
        ...
        public QuartoResponse atualizar(Long id, QuartoRequest req) {
                ...
                if (req.status() != null) {
                        q.setStatus(
                                        StatusQuarto.fromLabel(req.status()));
                }
                ...
                if (req.residenciaId() != null) {
                        q.setResidencia(
                                        residenciaService.buscarEntidade(
                                                        req.residenciaId()));
                }
                ...
                if (req.adicionalCamaExtra() != null) {
                        q.setAdicionalCamaExtra(
                                        req.adicionalCamaExtra());
                }

                if (req.percentualHospede() != null) {
                        q.setPercentualHospede(
                                        req.percentualHospede());
                }
        ...
        private void validarRegras(Quarto q) {

                CapacidadeHospedesRules.ConfiguracaoCapacidade config = switch (q.getTipo()) {

                        case INDIVIDUAL ->
                                CapacidadeHospedesRules.ConfiguracaoCapacidade
                                                .individual(
                                                                q.getNumCamas());
                ...
                int capacidadeMaxima = CapacidadeHospedesRules.calcularMaxima(
                                q.getTipo(),
                                config);

                CapacidadeHospedesRules.validarNumeroHospedes(
                                q.getNumeroHospedes(),
                                capacidadeMaxima);
        ...
        private Set<Comodidade> resolverComodidades(
                        List<Long> ids) {
                ...
                if (ids != null) {
                        ...
                        for (Long cid : ids) {
                                ...
                                set.add(
                                                comodidadeRepository.findById(cid)
                                                                .orElseThrow(
                                                                                () -> ResourceNotFoundException.of(
                                                                                                "Comodidade",
                                                                                                cid)));
        ...
        private QuartoResponse toResponse(Quarto q) {
                ...
                for (Comodidade c : q.getComodidades()) {
                        comodidades.add(
                                        new QuartoResponse.ComodidadeQuarto(
                                                        c.getNome(),
                                                        true));
                }
                ...
                return new QuartoResponse(
                                ...
                                q.getNome(),
                                q.getTipo().getLabel(),
                                q.getResidencia() != null
                                                ? q.getResidencia().getId()
                                                : null,
                                q.getResidencia() != null
                                                ? q.getResidencia().getNome()
                                                : null,
                                q.getValorBase(),
                                preco,
